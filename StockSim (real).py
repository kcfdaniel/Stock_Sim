...
class transactionWindow(QMainWindow):
    ...
    def keyPressEvent(self, e):
        if e.key() == QtCore.Qt.Key_Delete or e.key() == QtCore.Qt.Key_Backspace:
            ...
            self.data.to_csv('transactions.txt', sep='\t', index=False, header=False)
            ...
            if self.execute == True:
                while len(self.data.index) > 0:
                    ...
                    if not self.stockFrame.loc[self.stockFrame['name'] == name, 'price'].empty:
                        self.stockFrame.loc[self.stockFrame['name'] == name, 'price'] = self.stockFrame.loc[self.stockFrame['name'] == name, 'price'] + price
                    ...
                    self.change = True
                    # print("10")
                self.stockFrame.to_csv('stocks.txt', sep='\t', index=False, header=False)
                self.execute = False
